...
public partial class Admin_ProdEdit : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //MasterPage objMaster = Page.Master;
        //ScriptManager smsiteMaster = objMaster.FindControl("smsiteMaster") as ScriptManager;
        //if (smsiteMaster.IsInAsyncPostBack)
        //{
    ...
    protected void grdProdDes_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        try
        {
            ...
            TextBox txtProdDescription = gvr.FindControl("txtProdDescription") as TextBox;
            RadioButtonList rdbStatus = gvr.FindControl("rdbStatus") as RadioButtonList;
            HiddenField hdnProdDes = gvr.FindControl("hdnProdDes") as HiddenField;
            txtProdDescription.Text = Request.Form[hdnProdDes.UniqueID];
            SqlParameter[] paras = new SqlParameter[]{
    ...
    protected void lnkDeletMeta_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            GridViewRow gvr = grdProdDes.Rows[i] as GridViewRow;
            HiddenField hdnProductDesID = gvr.FindControl("hdnProductDesID") as HiddenField;
            SqlParameter[] paras = new SqlParameter[]{
                    new SqlParameter("@prodDescriptionId",hdnProductDesID.Value)
                };
            objDataAccess.DaExecNonQueryStr("Delete from productDescription  WHERE prodDescriptionId = @prodDescriptionId ", paras, CommandType.Text);
            BindGridProductDescription();
        }
        catch (Exception)
        {
        ...
        try
        {
            ...
            foreach (GridViewRow grRow in grdProdDes.Rows)
            {
                if (grRow.RowType == DataControlRowType.DataRow)
                {
                    DataRow dr = dt.NewRow();
                    dr["Srno"] = ((Label)grRow.FindControl("lblSrNo")).Text;
                    dr["Heading"] = ((Label)grRow.FindControl("lbldisHeading")).Text;
                    dr["Description"] = HttpUtility.HtmlEncode(((Label)grRow.FindControl("lblProdDes")).Text);
                    dr["activeflag"] = "1";
                    dt.Rows.Add(dr);
                }
            }
    ...
    protected void btnUpdateMetaTag_Click(object sender, EventArgs e)
    {
        //hdnMetaId.Value = "1";
        try
        {
            if (hdnMetaId.Value != "")
            {
                ...
                string SqlQry = @"delete product_Meta_trn where productId=@productId; 
                                  insert into product_Meta_trn(productId, MetaName, MetaContent,createdby, modifiedby,activeflag) 
                                  values(@productId, @MetaName, @MetaContent, @UserID,@UserID,1)";
                objDataAccess.DaExecNonQueryStr(SqlQry, paras, CommandType.Text);
                AlertMsg("Updated Successfuly");
            }
        }
